...
DOC
      ...
      def success?
        summary.offence_count < 1
      end

      def offenses
        files.each_with_object([]) do |file, issues|
          file['offenses'].each do |offense|
            location = {'line' => offense['lineNumber'], 'column' => '?'}
            issues << ::Rubocop::Offense.new(path: file['path'], location: location, cop_name: offense['name'], message: offense['message'])
          end
        end
      end
